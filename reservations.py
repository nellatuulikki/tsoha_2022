...
def get_available_rooms(reservation_date, hotel_id):
    sql = "SELECT b.id, room_description, guests, square_meters, hotel_id FROM calendar a left join rooms b on (a.room_id = b.id) where reservation_date = :reservation_date and hotel_id = :hotel_id"
...
def add_new_dates_to_calendar(start_date, end_date, room_id, available_rooms):
    ...
    try:
        while reservation_date <= end_date:
            sql = "INSERT INTO calendar (reservation_date, room_id, available_rooms) VALUES (:reservation_date, :room_id, :available_rooms)"
